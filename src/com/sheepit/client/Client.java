...
public class Client {
	...
	public synchronized int stop() {
		...
		if (this.server.getPage("logout").isEmpty() == false) {
			this.gui.status("Disconnecting from server");
	...
	protected int prepareWorkingDirectory(Job ajob) throws FermeExceptionNoSpaceLeftOnDevice {
		...
		if (renderer_path_file.exists()) {
			...
			if (ret != 0) {
				...
				this.gui.error(String.format("Unable to extract the renderer (error 1-%d)", ret));
				return -1;
			}
		...
		if (scene_path_file.exists()) {
			...
			if (ret != 0) {
				...
				this.gui.error(String.format("Unable to extract the scene (error 2-%d)", ret));
				return -2;
			}
		}
